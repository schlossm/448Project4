...
public class Catalog implements GlobalConst
{
	...
	//MARK: - Stuff for Part 2

	private RID ridForCurrentCountForTable(String fileName)
	{
		return getFileRID(fileName, true);
	}

	private int recCountForTable(String fileName)
	{
		RID rid = ridForCurrentCountForTable(fileName);
		Tuple currentCount = new Tuple(Minibase.SystemCatalog.s_rel, f_rel.selectRecord(rid));
		return (int)currentCount.getField("recCount");
	}

	void updateRecordCountInTableBy(String fileName, int count)
	{
		RID rid = ridForCurrentCountForTable(fileName);
		Tuple newRecCount = new Tuple(Minibase.SystemCatalog.s_rel);
		newRecCount.setField("recCount", recCountForTable(fileName) + count);
		newRecCount.setField("relName", fileName);
		f_rel.updateRecord(rid, newRecCount.getData());
	}
	...
	public void createTable(String fileName, Schema schema)
	{
		...
		RID rid = tuple.insertIntoFile(f_rel);
